...
BOOL WINAPI ConsoleHandler(DWORD signal) {
    if (signal == CTRL_C_EVENT) {
        // Signal your program to exit gracefully
        stopProgram = true;
        cv.notify_all();
        return TRUE;
    }
...
//    av_dict_set(&opts, "rc", "constqp", 0);
    ...
    av_dict_set(&opts, "preset", "p5", 0);   //p4
...
void recordingThread(int width, int height) {
    ...
    FrameBuffer frameBuffer(90); // Optimized buffer with max 90 frames.
    ...
    std::thread captureThread([&]() {
        while (isRecording) {
            uint8_t* buffer = new uint8_t[width * height * 4]; // Allocate a new frame buffer.
            ...
            // Wait if necessary to stabilize frame interval (targeting a certain FPS).
            int64_t waitTime = (1000000 / 90) - captureDuration;
            if (waitTime > 0) {
...
// Low-level keyboard hook to toggle recording on 'U' key press.
LRESULT CALLBACK LowLevelKeyboardProc(int nCode, WPARAM wParam, LPARAM lParam) {
    if (nCode == HC_ACTION) {
        KBDLLHOOKSTRUCT* kbStruct = (KBDLLHOOKSTRUCT*)lParam;
        if (kbStruct->vkCode == 0x55) { // 'U' key
...
// Thread-safe flags for controlling the program state.
//std::atomic_bool stopProgram{ false };
...
int main() {
    ...
    // Main message loop.
    MSG msg;
    while (GetMessage(&msg, NULL, 0, 0) && !stopProgram) {
